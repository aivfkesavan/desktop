...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("nidum", process.execPath, [
              ...
              click: () => {
                ...
                dialog
                  .showMessageBox({
                    ...
                    title: "Help",
                    message:
                      "For assistance, you can reach us at info@nidum.ai or visit our website.",
...
autoUpdater.on("download-progress", (progressObj) => {
  ...
  log.info(
    `${progressObj.transferred} bytes out of ${progressObj.total} bytes.`
  );
});
  ...
  dialog
    ...
    .then((returnValue) => {
      if (returnValue.response === 0) {
        log.info(
          "User chose to restart the application to install the update."
        );
